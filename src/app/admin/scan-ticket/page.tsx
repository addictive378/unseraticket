...
import { ScanLine, CheckCircle2, XCircle, AlertTriangle, RotateCcw, Camera, CameraOff } from 'lucide-react';
    ...
    return (
        <div>
            <div className={styles.pageHeader}>
                <div>
                    <h1>Scan Tiket</h1>
                    <p>Scan QR code tiket untuk verifikasi masuk event.</p>
                </div>
            </div>
            ...
            <div style={{ maxWidth: '600px', margin: '2rem auto' }}>
                    ...
                    <div className="glass" style={{ borderRadius: '20px', overflow: 'hidden' }}>
                        ...
                        {!scanning && (
                            <div style={{ padding: '3rem', textAlign: 'center' }}>
                                <ScanLine size={56} style={{ color: 'var(--primary)', marginBottom: '1.5rem' }} />
                                <h3 style={{ marginBottom: '0.75rem' }}>Scanner QR Tiket</h3>
                                <p style={{ color: 'rgba(255,255,255,0.5)', marginBottom: '2rem', fontSize: '0.9rem' }}>
                                    Arahkan kamera ke QR code tiket untuk verifikasi masuk
                                </p>
                ...
                {!scanResult && (
                    <div className="glass" style={{ borderRadius: '16px', padding: '1.5rem', marginTop: '1.5rem' }}>
                        <h4 style={{ marginBottom: '0.75rem', fontSize: '0.9rem', color: 'rgba(255,255,255,0.6)' }}>Atau masukkan kode manual:</h4>
                        <form onSubmit={handleManualScan} style={{ display: 'flex', gap: '0.75rem' }}>
                            <input
                                type="text"
                                value={manualCode}
                                onChange={(e) => setManualCode(e.target.value)}
                                placeholder="VIBRANT-TIKET:xxxxx"
                ...
                {processing && (
                    <div className="glass" style={{ borderRadius: '20px', padding: '3rem', textAlign: 'center', marginTop: '1.5rem' }}>
                        <div className={styles.loading}>Memverifikasi tiket...</div>
                    </div>
                )}
                    ...
                    <div className="glass" style={{ borderRadius: '20px', overflow: 'hidden' }}>
                            ...
                            <div style={{ padding: '1.5rem' }}>
                                <div style={{ display: 'grid', gap: '1rem' }}>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', padding: '0.75rem 0', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
                                        <span style={{ color: 'rgba(255,255,255,0.4)', fontSize: '0.85rem' }}>ID Tiket</span>
                                        <span style={{ fontFamily: 'monospace', color: 'var(--primary)', fontSize: '0.85rem' }}>{scanResult.ticket.id}</span>
                                    </div>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', padding: '0.75rem 0', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
                                        <span style={{ color: 'rgba(255,255,255,0.4)', fontSize: '0.85rem' }}>Pemegang</span>
                                        <span style={{ fontWeight: 600 }}>{scanResult.ticket.holderName}</span>
                                    </div>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', padding: '0.75rem 0', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
                                        <span style={{ color: 'rgba(255,255,255,0.4)', fontSize: '0.85rem' }}>Acara</span>
                                        <span style={{ fontWeight: 600 }}>{scanResult.ticket.eventName}</span>
                                    </div>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', padding: '0.75rem 0' }}>
                                        <span style={{ color: 'rgba(255,255,255,0.4)', fontSize: '0.85rem' }}>Tipe</span>
                                        <span style={{ fontWeight: 600 }}>{scanResult.ticket.ticketType}</span>
                                    </div>
                                </div>
                            </div>
                        )}
                        ...
                        <div style={{ padding: '1.5rem', borderTop: '1px solid rgba(255,255,255,0.06)', textAlign: 'center' }}>
                            <button onClick={resetScan} className="btn-primary" style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem' }}>
                                <RotateCcw size={16} /> SCAN TIKET LAIN
                            </button>
                        </div>
                    </div>
                )}
